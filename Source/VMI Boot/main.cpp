...
int main(int argc, char *argv[]){
    ...
    auto *boot_file = new TFile((root_file.substr(0,root_file.size()-5)+std::string("_boot.root")).c_str(), "recreate");
    ...
    std::ofstream file;
    std::string s(root_file); s.resize(s.size()-5);
    s += ".out";
    file.open(s.c_str());
    ...
    std::vector<double> ref_radial(resolution);
    std::vector<double> sample_index;
    std::vector<double> I_iter(nr_samples*resolution);
    std::vector<double> z(resolution); //Bias correction
    std::vector<double> iteration(nr_samples);
    std::vector<double> I_lower(resolution,0);
    std::vector<double> I_upper(resolution,0);
    ...
    injection_branch->GetEntry(tree->LoadTree(entries-1));
    int max_injection = injection;

    // If event_time is not set, look for the maximum time
    if (!event_time_set) {
        max_branch->GetEntry(3);
        settings_time_vec.at(3) = max_val;
    }
    if (!peak_sum_set) {
    ...
    std::vector<double> angular_bin_val(nr_angular_bins);
    ...
    file << "Iteration on raw data" << std::endl;
    {
        /*--------            Finding center             --------*/

        //If center has been calculated and resolution has not changed just read the values
        if (data_file->GetListOfKeys()->Contains("CENTER") && !resolution_changed && !center_mode_changed) {
            double center;
                ...
                for (int i = 0; i < resolution * norder; ++i) {
                    ...
                    rad_bin_err.at(i) = rad_bin_shutter_on_err_first.at(i) + pow(injections_laser_on / injections_laser_off,2.) *
                                        ...
                                        pow(injections_time_first / injections_time_last, 2.) * (
                                                rad_bin_shutter_on_err_last.at(i) + pow(injections_laser_on / injections_laser_off,2.) *
        ...
        auto *I_ref = new TTree("INTENSITY", "INTENSITY");
        auto *R_ref = new TTree("RADIUS", "RADIUS");
        ...
        I_ref->Branch("intensity", &I_val, "intensity/D");
        R_ref->Branch("radius", &R_val, "radius/D");
        I_err_tree->Branch("error", &I_err_val, "error/D");

        for (int i = 0; i < resolution; ++i) {
            I_val = I.at(i); intensity.at(i) = I.at(i);
            I_ref->Fill(); file << I_val << " ";
        } file << std::endl;
        for (int i = 0; i < resolution; ++i) {
            I_err_val = I_err.at(i);
            I_err_tree->Fill(); file << I_err_val << " ";
        } file << std::endl;
        for (int i = 0; i < resolution; ++i) {
            R_val = rad_bin_val.at(i); ref_radial.at(i) = rad_bin_val.at(i);
            R_ref->Fill();  file << R_val << " ";
        } file << std::endl;


        I_ref->Write(nullptr, TObject::kOverwrite);
        R_ref->Write(nullptr, TObject::kOverwrite);
        I_err_tree->Write(nullptr, TObject::kOverwrite);
        data_file->cd();
    }

    /*--------            Sampling with replacement             --------*/
    file << "Sampling" << std::endl;
    // Multithread or not
    if (nr_threads > 1) {
        for (int sample = 0; sample < nr_samples; ++sample) {
            ...
            ROOT::TThreadedObject<double> th_sum_x_obj(0.);
            ROOT::TThreadedObject<double> th_sum_y_obj(0.);
            ROOT::TThreadedObject<double> th_count_x_obj(0.);
            ROOT::TThreadedObject<double> th_injection_on_obj(0.);
            ROOT::TThreadedObject<double> th_injection_off_obj(0.);
            ROOT::TThreadedObject<std::vector<double>> th_weights_obj(max_injection,
                                                                      0.); //Could be large and fill up memory

            // The multi-threaded reader of the TTree
            auto thread_center_read = [&](TTreeReader &reader) {
                auto th_sum_x = th_sum_x_obj.Get();
                auto th_sum_y = th_sum_y_obj.Get();
                auto th_count_x = th_count_x_obj.Get();
                auto th_injection_on = th_injection_on_obj.Get();
                auto th_injection_off = th_injection_off_obj.Get();
                auto th_weights = th_weights_obj.Get();
                ...
                while (reader.Next()) {
                    ...
                    auto inject = *injection_reader;
                    inject -= 1;

                    th_weights->at(inject) = weight(gen); //Draw random weight to data point

                    // Add to cumulative injection count
                    if (shutter) {
                        *th_injection_on += th_weights->at(inject);
                    } else {
                        *th_injection_off += th_weights->at(inject);
                    }

                    // project the data on the axis
                    for (int j = 0; j < X.size(); j++) {
                        // Define time window
                        if (trig_time.at(j) < trig_time_upper && trig_time.at(j) > trig_time_lower &&
                            event_time.at(j) < event_time_upper && event_time.at(j) > event_time_lower) {

                            sum_x += X.at(j) * th_weights->at(inject);
                            count_x += th_weights->at(inject);

                            sum_y += Y.at(j) * th_weights->at(inject);
                        }
                    }
            ...
            tp.Process(thread_center_read);

            // Merge threads
            for (int i = 0; i < nr_threads; ++i) {
                // Merge sums on axis
                auto ent = th_sum_x_obj.GetAtSlot(i);
                sum_x += *ent;
                ent = th_sum_y_obj.GetAtSlot(i);
                sum_y += *ent;
                ent = th_count_x_obj.GetAtSlot(i);
                count_x += *ent;

                //Merge injection counts
                ent = th_injection_on_obj.GetAtSlot(i);
                injections_laser_on += *ent;
                ent = th_injection_off_obj.GetAtSlot(i);
                injections_laser_off += *ent;

                // Merge weights
                auto st = *th_weights_obj.GetAtSlot(i);
                for (int j = 0; j < entries; ++j) {
                    if (st.at(j) != 0) {
                        weights.at(j) = st.at(j);
                    }
                }
            }
            ...
            if (circularise) {
                *//*--------            Circularize image             --------*//*
                ...
                // Creating thread safe objects
                ROOT::TThreadedObject<std::vector<double>> th_circ_rad_bin_on_obj(resolution * nr_angular_bins, 0.);
                ROOT::TThreadedObject<std::vector<double>> th_circ_rad_bin_off_obj(resolution * nr_angular_bins,
                                                                                   0.);

                // The multi-threaded reader of the TTree
                auto thread_circular_read = [&](TTreeReader &reader) {
                    auto th_circ_rad_bin_on = th_circ_rad_bin_on_obj.Get();
                    auto th_circ_rad_bin_off = th_circ_rad_bin_off_obj.Get();
                        ...
                        if (shutter) {
                            for (int j = 0; j < X.size(); j++) {
                                ...
                                if (trig_time.at(j) < trig_time_upper && trig_time.at(j) > trig_time_lower &&
                                    ...
                                    // Calculate the radius
                                    th_hold_r = pow((X.at(j) - x_center) * (X.at(j) - x_center) +
                                                    (Y.at(j) - y_center) * (Y.at(j) - y_center), 1. / 2.);
                                    ...
                                    //Find the bin where the theta lies in
                                    if ((X.at(j) - x_center) >= 0) { //Upper plane
                                        auto lower_t = std::lower_bound(angular_bin_val.begin(),
                                                                        angular_bin_val.end(),
                                                                        std::acos(
                                                                                (Y.at(j) - y_center) / th_hold_r));
                                    ...
                                    // Add to bin
                                    th_circ_rad_bin_on->at(th_ang_index * resolution + th_index - 1) +=
                                            (th_hold_r - rad_bin_val.at(th_index - 1)) / rad_bin_size *
                                            weights.at(inject);
                                    th_circ_rad_bin_on->at(th_ang_index * resolution + th_index) +=
                                            (1. + rad_bin_val.at(th_index - 1) - th_hold_r) / rad_bin_size *
                                            weights.at(inject);
                                }
                            }
                        } else {
                            for (int j = 0; j < X.size(); j++) {
                                ...
                                if (trig_time.at(j) < trig_time_upper && trig_time.at(j) > trig_time_lower &&
                                    ...
                                    // Calculate the radius
                                    hold_r = pow((X.at(j) - x_center) * (X.at(j) - x_center) +
                                                 (Y.at(j) - y_center) * (Y.at(j) - y_center), 1. / 2.);
                                    ...
                                    if ((X.at(j) - x_center) >= 0) { //Upper plane
                                        auto lower_t = std::lower_bound(angular_bin_val.begin(),
                                                                        angular_bin_val.end(),
                                                                        std::acos((Y.at(j) - y_center) / hold_r));
                                        ang_index = (int) std::distance(angular_bin_val.begin(), lower_t);
                                    } else { //Lower plane
                                        auto lower_t = std::lower_bound(angular_bin_val.begin(),
                                                                        angular_bin_val.end(),
                                    ...
                                    // Add to bin
                                    th_circ_rad_bin_off->at(ang_index * resolution + index - 1) -=
                                            (hold_r - rad_bin_val.at(index - 1)) / rad_bin_size *
                                            weights.at(inject);
                                    th_circ_rad_bin_off->at(ang_index * resolution + index) -=
                                            (1. + rad_bin_val.at(index - 1) - hold_r) / rad_bin_size *
                                            weights.at(inject);
                                }
                            }
                ...
                tp.Process(thread_circular_read);

                // Merge threads
                for (int i = 0; i < nr_threads; ++i) {
                    // Merge vectors
                    auto st = *th_circ_rad_bin_on_obj.GetAtSlot(i);
                    for (int j = 0; j < resolution * nr_angular_bins; ++j) {
                        circ_rad_bin_on.at(j) += st.at(j);
                    }
                    st = *th_circ_rad_bin_off_obj.GetAtSlot(i);
                    for (int j = 0; j < resolution * nr_angular_bins; ++j) {
                        circ_rad_bin_off.at(j) += st.at(j);
                    }
                }
            ...
            */
            ...
            for (int i = 0; i < resolution; ++i) {
                ...
                file << spline->Eval(ref_radial.at(i)) << " ";
            }
            file << std::endl;
    ...
    std::cout << "Calculating CI" << std::endl;
    count = 0;
    ...
    auto *I_err_l_tree = new TTree("INTENSITY_ERROR_LOWER", "INTENSITY_ERROR_LOWER");
    auto *I_err_u_tree = new TTree("INTENSITY_ERROR_UPPER", "INTENSITY_ERROR_UPPER");
    auto *I_iter_tree = new TTree("BOOTSTRAP_ITERATIONS", "BOOTSTRAP_ITERATIONS");

    I_err_l_tree->Branch("intensity_err", &I_val_err_l, "intensity_err/D");
    I_err_u_tree->Branch("intensity_err", &I_val_err_u, "intensity_err/D");
    I_iter_tree->Branch("iteration", &I_iter_val, "iteration/D");
    ...
    I_err_l_tree->Write(nullptr, TObject::kOverwrite);
    I_err_u_tree->Write(nullptr, TObject::kOverwrite);
    I_iter_tree->Write(nullptr, TObject::kOverwrite);
